	...
	for (i = 0; i < core->blocksize; i += element_size) {
		...
		if (n >= a && n <= b) {
			...
			r_cons_printf (core->cons, "Cd %u @ 0x%" PFMT64x ":%u\n",
				(ut32)element_size, core->addr + i, (ut32)element_size);
			// TODO: add data xrefs
		}
	}
}
...
#define return_tail(x) { tail_return_value = x; goto tail_return; }
	...
	// int dataAlign = r_arch_info (esil->anal->arch, R_ARCH_INFO_DATA_ALIGN);
	int codeAlign = r_arch_info (esil->anal->arch, R_ARCH_INFO_CODE_ALIGN);
	...
	for (; true; r_anal_op_fini (&op)) {
		...
		if (until_addr != UT64_MAX && addr == until_addr) {
			return_tail (0);
		}
		if (r_cons_is_breaked (core->cons)) {
...
static void cmd_anal_info(RCore *core, const char *input) {
	...
	case 'a': // "aia"
		if (input[1] == 'j') { // "aiaj"
			PJ *pj = r_core_pj_new (core);
			pj_o (pj);
			int v = r_arch_info (core->anal->arch, R_ARCH_INFO_MINOP_SIZE);
			pj_ki (pj, "minopsz", v);
			v = r_arch_info (core->anal->arch, R_ARCH_INFO_MAXOP_SIZE);
			pj_ki (pj, "maxopsz", v);
			v = r_arch_info (core->anal->arch, R_ARCH_INFO_INVOP_SIZE);
			pj_ki (pj, "invopsz", v);
			v = r_arch_info (core->anal->arch, R_ARCH_INFO_DATA_ALIGN);
			pj_ki (pj, "dtalign", v);
			v = r_arch_info (core->anal->arch, R_ARCH_INFO_CODE_ALIGN);
			...
			if (ops) {
				...
				while (nl) {
					*nl = 0;
					...
					if (desc) {
						char padstr[20];
						const char *pad = r_str_pad (padstr, sizeof (padstr), ' ', 16 - strlen (ptr));
...
static void aav_cb(RCore *core, ut64 from, ut64 to, int vsize, void *user) {
	...
	const int arch_align = r_arch_info (core->anal->arch, R_ARCH_INFO_CODE_ALIGN);
...
static void cmd_anal_aav(RCore *core, const char *input) {
	...
	int archAlign = r_arch_info (core->anal->arch, R_ARCH_INFO_CODE_ALIGN);
...
static bool match_prelude_internal(RCore *core, const char *input, ut64 *fcnaddr, const ut8 *mask, int masklen) {
	...
	int fcnalign = r_arch_info (core->anal->arch, R_ARCH_INFO_FUNC_ALIGN);
	if (fcnalign < 2) {
		fcnalign = 4;
	}
