...
static inline RXmlRet xml_elemstart(RXml *x, unsigned ch) {
	return r_xml_pushstack (x, &x->elem, ch);
}
...
static inline RXmlRet r_xml_attrstart(RXml *x, unsigned ch) {
	return r_xml_pushstack (x, &x->attr, ch);
}

static inline RXmlRet r_xml_pistart(RXml *x, unsigned ch) {
	return r_xml_pushstack (x, &x->pi, ch);
}
static inline RXmlRet r_xml_piname(RXml *x, unsigned ch) {
	return r_xml_pushstackc (x, ch);
}
static inline RXmlRet r_xml_piabort(RXml *x, unsigned ch) {
	r_xml_popstack (x);
	return R_XML_OK;
}
...
static inline RXmlRet r_xml_pivalend(RXml *x, unsigned ch) {
	r_xml_popstack (x);
	x->pi = (char *)x->stack;
	return R_XML_PIEND;
}
...
static RXmlRet r_xml_refend(RXml *x, RXmlRet ret) {
	...
	if (*r == '#') {
		...
		ch = '>';
	} else if (!strcmp (r, "amp")) {
		ch = '&';
	} else if (!strcmp (r, "apos")) {
		ch = '\'';
	} else if (!strcmp (r, "quot")) {
		ch = '"';
	}
...
R_API bool r_xml_init(RXml *x, size_t stacksize) {
	R_RETURN_VAL_IF_FAIL (x, false);
	memset (x, 0, sizeof (*x));
	x->stack = malloc (stacksize);
	if (!x->stack) {
		return false;
	}
	x->line = 1;
...
SP_CASES:
	...
	case R_XML_STATE_ATTR4:
		...
		if (ch == ';') {
			x->state = R_XML_STATE_ATTR3;
			return r_xml_refend (x, R_XML_ATTRVAL);
		}
		break;
	...
	case R_XML_STATE_COMMENT3:
		x->state = (ch == '-')? R_XML_STATE_COMMENT4: R_XML_STATE_COMMENT2;
		return R_XML_OK;
...
SP_CASES:
	...
	case R_XML_STATE_MISC2a:
		...
		if (ch == ';') {
			x->state = R_XML_STATE_MISC2;
			return r_xml_refend (x, R_XML_CONTENT);
		}
		break;
...
static void xml_indent(RStrBuf *sb, int level) {
	int i;
	for (i = 0; i < level; i++) {
		r_strbuf_append (sb, "  ");
	}
