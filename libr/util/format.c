...
static void r_print_format_num(RPrintFormat *pf, const char *setval, ut64 seeki, ut8 *buf, int i, int bytes, int sign, int size) {
	...
	if (bytes == 1) {
		...
		addr = r_read_ble16 (buf + i, endian);
	} else if (bytes == 4) {
		addr = r_read_ble32 (buf + i, endian);
	} else {
		addr = r_read_ble64 (buf + i, endian);
	}
	if (MUSTSET) {
		...
		if (size == -1) {
			if (sign) {
				st64 saddr = 0;
				switch (bytes) {
				case 1: saddr = (st8) addr; break;
				case 2: saddr = (st16) addr; break;
				case 4: saddr = (st32) addr; break;
				default: saddr = (st64) addr; break;
				}
				pj_kN (pf->pj, "value", saddr);
			} else {
				pj_kn (pf->pj, "value", addr);
			}
		} else {
...
R_API const char *r_print_format_byname(RPrint *p, const char *name) {
	return p? sdb_const_get (p->formats, name, NULL): NULL;
}
...
#define ISNESTED ((slide % STRUCTPTR) <= (oldslide % STRUCTPTR))
	...
	for (; times; times--) { // repeat N times
		...
		feed_me_again:
			...
			bool noline = false;
			bool json_field_closed = false;

			if (isptr == PF_PTR_NULL) {
				if (MUSTSEEJSON) {
					pj_ks (pf->pj, "value", "NULL");
					pj_end (pf->pj);
					json_field_closed = true;
				} else if (MUSTSEE) {
					r_print_printf (p, " NULL\n");
				}
				noline = true;
				...
				case '?':
					...
					if (MUSTSEE && !SEEVALUE) {
						...
						if (size == -1) {
							...
							if ((mode & R_PRINT_ISFIELD) && !(mode & R_PRINT_JSON)) {
								if (!SEEVALUE) {
									r_print_printf (p, "[\n");
								}
							}
			...
			} else if (pf->pj && mode & R_PRINT_JSON && !json_field_closed) {
				pj_end (pf->pj);
			} else if (mode & R_PRINT_DOT) {
				r_print_printf (p, "}");
