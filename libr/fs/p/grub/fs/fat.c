...
#define GRUB_FAT_ATTR_LONG_NAME (GRUB_FAT_ATTR_READ_ONLY | GRUB_FAT_ATTR_HIDDEN | GRUB_FAT_ATTR_SYSTEM | GRUB_FAT_ATTR_VOLUME_ID)
#define GRUB_FAT_ATTR_VALID (GRUB_FAT_ATTR_READ_ONLY | GRUB_FAT_ATTR_HIDDEN | GRUB_FAT_ATTR_SYSTEM | GRUB_FAT_ATTR_DIRECTORY | GRUB_FAT_ATTR_ARCHIVE | GRUB_FAT_ATTR_VOLUME_ID)
R_PACKED(
	struct grub_fat_bpb {
...
grub_fat_mount(grub_disk_t disk) {
	...
	if (strncmp ((const char *)bpb.version_specific.fat12_or_fat16.fstype, "FAT12", 5) && strncmp ((const char *)bpb.version_specific.fat12_or_fat16.fstype, "FAT16", 5) && strncmp ((const char *)bpb.version_specific.fat32.fstype, "FAT32", 5)) {
		goto fail;
	}
	...
	data->num_root_sectors = ((((grub_uint32_t)grub_le_to_cpu16 (bpb.num_root_entries) * GRUB_FAT_DIR_ENTRY_SIZE + grub_le_to_cpu16 (bpb.bytes_per_sector) - 1) >> (data->logical_sector_bits + GRUB_DISK_SECTOR_BITS))
		<< (data->logical_sector_bits));

	data->cluster_sector = data->root_sector + data->num_root_sectors;
	data->num_clusters = (((data->num_sectors - data->cluster_sector) >> (data->cluster_bits + data->logical_sector_bits)) + 2);
...
static grub_ssize_t
grub_fat_read_data(grub_disk_t disk, struct grub_fat_data *data, void(*read_hook)(grub_disk_addr_t sector, unsigned offset, unsigned length, void *closure), void *closure, int flags, grub_off_t offset, grub_size_t len, char *buf) {
	grub_size_t size;
	...
	logical_cluster_bits = (data->cluster_bits + data->logical_sector_bits + GRUB_DISK_SECTOR_BITS);
	...
	while (len) {
		while (logical_cluster > data->cur_cluster_num) {
			...
			/* Read the FAT.  */
			if (grub_disk_read (disk, data->fat_sector, fat_offset, (data->fat_size + 7) >> 3, (char *)&next_cluster)) {
				return -1;
			}
		...
		/* Read the data here.  */
		sector = (data->cluster_sector + ((data->cur_cluster - 2) << (data->cluster_bits + data->logical_sector_bits)));
...
static grub_err_t
grub_fat_iterate_dir(grub_disk_t disk, struct grub_fat_data *data, int(*hook)(const char *filename, struct grub_fat_dir_entry *dir, void *closure), void *closure) {
	...
	for (offset = 0;; offset += sizeof (dir)) {
		...
		if ((grub_fat_read_data (disk, data, 0, 0, 0, offset, sizeof (dir), (char *)&dir) != sizeof (dir) || dir.name[0] == 0)) {
			break;
		}
		/* Handle long name entries.  */
		if (dir.attr == GRUB_FAT_ATTR_LONG_NAME) {
			struct grub_fat_long_name_entry *long_name = (struct grub_fat_long_name_entry *)&dir;
		...
		if (checksum != -1 && slot == 0) {
			...
			if (sum == checksum) {
				...
				*grub_utf16_to_utf8 ((grub_uint8_t *)filename, unibuf, slots * 13) = '\0';
		...
		if (dir.attr & GRUB_FAT_ATTR_VOLUME_ID) {
			for (i = 0; i < sizeof (dir.name) && dir.name[i] && !grub_isspace (dir.name[i]); i++) {
				*filep++ = dir.name[i];
			}
		} else {
...
static int
grub_fat_find_dir_hook(const char *filename, struct grub_fat_dir_entry *dir, void *closure) {
	...
	if (grub_strcasecmp (c->dirname, filename) == 0) {
		...
		data->file_cluster = ((grub_le_to_cpu16 (dir->first_cluster_high) << 16) | grub_le_to_cpu16 (dir->first_cluster_low));
		data->cur_cluster_num = ~0U;
...
static grub_err_t
grub_fat_dir(grub_device_t device, const char *path, int(*hook)(const char *filename, const struct grub_dirhook_info *info, void *closure), void *closure) {
	struct grub_fat_data *data = 0;
...
grub_fat_read(grub_file_t file, char *buf, grub_size_t len) {
	return grub_fat_read_data (file->device->disk, file->data, file->read_hook, file->closure, file->flags, file->offset, len, buf);
}
...
static int
grub_fat_label_hook(const char *filename, struct grub_fat_dir_entry *dir, void *closure) {
	char **label = closure;
