...
static int grub_hfs_find_node(struct grub_hfs_data *, char *, grub_uint32_t, int, char *, int, int);
...
static unsigned int
grub_hfs_block(struct grub_hfs_data *data, grub_hfs_datarecord_t dat, int file, int block, int cache, int depth) {
	...
	for (;;) {
		...
		for (i = 0; i < 3; i++) {
			...
			if (grub_be_to_cpu16 (dr[i].count) + pos > block) {
				...
				return (grub_be_to_cpu16 (data->sblock.first_block) + (first + block - pos) * GRUB_HFS_BLKS);
			}
		...
		key.first_block = grub_cpu_to_be16 (pos);
		tree = 1;
		grub_hfs_find_node (data, (char *)&key, data->ext_root, 1, (char *)&dr, sizeof (dr), depth);
		if (grub_errno) {
			return 0;
		}
	}
...
grub_hfs_read_file(struct grub_hfs_data *data,
	...
	blockcnt = ((len + pos) + data->blksz - 1) / data->blksz;
	...
	for (i = pos / data->blksz; i < blockcnt; i++) {
		...
		if (blknr) {
			...
			grub_disk_read (data->disk, blknr, skipfirst, blockend, buf);
...
grub_hfs_mount(grub_disk_t disk) {
	...
	/* Read the superblock.  */
	if (grub_disk_read (disk, GRUB_HFS_SBLOCK, 0, sizeof (struct grub_hfs_sblock), &data->sblock)) {
		goto fail;
	}
	...
	first_block = ((grub_be_to_cpu16 (data->sblock.extent_recs[0].first_block) * GRUB_HFS_BLKS) + grub_be_to_cpu16 (data->sblock.first_block));

	if (grub_disk_read (data->disk, first_block, 0, sizeof (treehead), &treehead)) {
		goto fail;
	}
	...
	first_block = ((grub_be_to_cpu16 (data->sblock.catalog_recs[0].first_block) * GRUB_HFS_BLKS) + grub_be_to_cpu16 (data->sblock.first_block));
	if (grub_disk_read (data->disk, first_block, 0, sizeof (treehead), &treehead)) {
		goto fail;
	}
...
grub_hfs_cmp_extkeys(struct grub_hfs_extent_key *k1,
	...
	if (cmp == 0) {
		cmp = (grub_be_to_cpu16 (k1->first_block) - grub_be_to_cpu16 (k2->first_block));
	}
	return cmp;
}
...
static grub_err_t
grub_hfs_iterate_records(struct grub_hfs_data *data, int type, int idx, int this, int(*node_hook)(struct grub_hfs_node *hnd, struct grub_hfs_record *, void *closure), void *closure, int depth) {
	...
	do {
		...
		int blk = grub_hfs_block (data, dat, (type == 0)? GRUB_HFS_CNID_CAT: GRUB_HFS_CNID_EXT, idx / divident, 0, depth);
...
grub_hfs_find_node_node_found(struct grub_hfs_node *hnd,
	...
	if (hnd->type == GRUB_HFS_NODE_LEAF) {
		...
		if (cmp == 0) {
			...
			grub_memcpy (c->datar, rec->data, rec->datalen < c->datalen? rec->datalen: c->datalen);
			return 1;
		}
	}
...
grub_hfs_iterate_dir_it_dir(struct grub_hfs_node *hnd,
	...
	if (grub_be_to_cpu32 (ckey->parent_dir) != grub_be_to_cpu32 ((origkey)->parent_dir)) {
		return 1;
	}
...
call HOOK.  */
static grub_err_t
grub_hfs_iterate_dir(struct grub_hfs_data *data, grub_uint32_t root_idx, unsigned int dir, int(*hook)(struct grub_hfs_record *, void *closure), void *closure, int depth) {
	...
	do {
		...
		if (grub_hfs_iterate_records (data, 0, root_idx, 0, grub_hfs_iterate_dir_node_found, &c, depth)) {
			return grub_errno;
		}
...
static grub_err_t
grub_hfs_find_dir(struct grub_hfs_data *data, const char *path, struct grub_hfs_filerec *retdata, int *retinode, int depth) {
	...
	while (path_iter && grub_strlen (path_iter)) {
		...
		/* Lookup this node.  */
		if (!grub_hfs_find_node (data, (char *)&key, data->cat_root, 0, (char *)&fdrec.frec, sizeof (fdrec.frec), depth)) {
...
grub_hfs_dir_hook(struct grub_hfs_record *rec, void *closure) {
	...
	if (*filetype == GRUB_HFS_FILETYPE_DIR || *filetype == GRUB_HFS_FILETYPE_FILE) {
...
static grub_err_t
grub_hfs_dir(grub_device_t device, const char *path, int(*hook)(const char *filename, const struct grub_dirhook_info *info, void *closure), void *closure) {
	int inode;
	int depth = 0;
	struct grub_hfs_data *data;
...
static grub_ssize_t grub_hfs_read(grub_file_t file, char *buf, grub_size_t len) {
	...
	int depth = 0;
	return grub_hfs_read_file (data, file->read_hook, file->closure, file->offset, len, buf, depth);
}
